<template>
	<div class="mt-5">
		<div class="text-darkmode-blue-contrast1 mb-10">
			Showing history of testcase <span class="italic font-bold">{{ testname }}</span>
		</div>

		<div class="col-span-1 text-xs my-auto">
			<input type="checkbox"
						 class="rounded mt-2"
						 v-model="showFailedOnly" /><span class="ml-1 text-darkmode-blue-contrast1">Show Failed only222</span>
						 <div class="border border-red-500">
			<InputText v-model="testname" value="AAAAAA" class="border-2 border-green-400"/>
		</div>
			<InputTextArea />
		</div>
	</div>
...
const testname = ref(useRoute().query["testname"]?.toString())
...
const highlight = function(line: string): string {
	const logline = searchTerm.value;
	if (!logline) {
		return line
	}
